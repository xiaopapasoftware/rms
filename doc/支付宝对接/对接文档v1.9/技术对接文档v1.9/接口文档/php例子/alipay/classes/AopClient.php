...
class AopClient {
	//应用ID
	public $appId = "2015122300879608";
	
	//私钥文件路径
	public $rsaPrivateKeyFilePath;

	//私钥值
	public $rsaPrivateKey = '';
	//网关
	public $gatewayUrl = "http://oepnapi.eco.dl.alipaydev.com/gateway.do";//测试
	//public $gatewayUrl = "https://openapipre.alipay.com/gateway.do";//预发
	//public $gatewayUrl = "https://openapi.alipay.com/gateway.do";//生产
	...
	//加密密钥和类型
	public $encryptKey = 'qWNyRTM76HojuQcLGBqYsA=='; //非生产
	//public $encryptKey = 'OFjw+p+lXidckub5aDa88A=='; //生产
	...
	//异步通知参数排序
	public function getSignContent2($params) {
		ksort($params);
	...
	*/
	public function pageExecute($request,$httpmethod = "POST") {
		...
		if (strcasecmp($this->fileCharset, $this->postCharset)) {
			...
			die("文件编码：[" . $this->fileCharset . "] 与表单提交编码：[" . $this->postCharset . "]两者不一致!");
		}
		...
		if (method_exists($request,"getNeedEncrypt") &&$request->getNeedEncrypt()){
			...
			if ($this->checkEmpty($apiParams['biz_content'])) {
				...
				die(" api request Fail! The reason : encrypt request is not supperted!");
			}

			if ($this->checkEmpty($this->encryptKey) || $this->checkEmpty($this->encryptType)) {

				die(" encryptType and encryptKey must not null! ");
			}
	...
	public function rsaCheckV3($params, $rsaPublicKeyFilePath, $signType='RSA') {
		$sign = $params['sign'];
		unset($params['sign']);
		return $this->verify($this->getSignContent2($params), $sign, $rsaPublicKeyFilePath, $signType);
	}
	...
	public function checkResponseSign($request, $signData, $resp, $respObject) {
		...
		if (!$this->checkEmpty($this->alipayPublicKey) || !$this->checkEmpty($this->alipayrsaPublicKey)) {
			...
			if ($signData == null || $this->checkEmpty($signData->sign) || $this->checkEmpty($signData->signSourceData)) {
				...
				die(" check sign Fail! The reason : signData is Empty");
			}
			...
			if (!$this->checkEmpty($responseSubCode) || ($this->checkEmpty($responseSubCode) && !$this->checkEmpty($signData->sign))) {
				...
				if (!$checkResult) {
					...
					if (strpos($signData->signSourceData, "\\/") > 0) {
						...
						if (!$checkResult) {
							die("check sign Fail! [sign=" . $signData->sign . ", signSourceData=" . $signData->signSourceData . "]");
						}
