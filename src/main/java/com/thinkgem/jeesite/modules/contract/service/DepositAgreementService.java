...
public class DepositAgreementService extends CrudService<DepositAgreementDao, DepositAgreement> {
	...
	if (ValidatorFlagEnum.SAVE.getValue().equals(depositAgreement.getValidatorFlag())) {
		...
		paymentTransService.generateAndSavePaymentTrans(TradeTypeEnum.DEPOSIT_AGREEMENT.getValue(), PaymentTransTypeEnum.RECEIVABLE_DEPOSIT.getValue(), id, TradeDirectionEnum.IN.getValue(), depositAgreement.getDepositAmount(), depositAgreement.getDepositAmount(), 0D, PaymentTransStatusEnum.NO_SIGN.getValue(), depositAgreement.getStartDate(), depositAgreement.getExpiredDate());
	    }
	    // 更新房屋/房间状态
	    if (RentModelTypeEnum.WHOLE_RENT.getValue().equals(depositAgreement.getRentMode())) {
		House house = houseService.get(depositAgreement.getHouse().getId());
