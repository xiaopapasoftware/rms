	...
	<script type="text/javascript">
		$(document).ready(function() {
			$("#btnExport").click(function(){
				$("#searchForm").attr("action","${ctx}/report/financial/exportPay");
				$("#searchForm").submit();
				$("#searchForm").attr("action","${ctx}/report/financial/pay");
			});
		});
		function page(n,s){
			$("#pageNo").val(n);
...
<body>
	<form:form id="searchForm" modelAttribute="payRentReport" action="${ctx}/report/financial/pay" method="post" class="breadcrumb form-search">
		...
		<ul class="ul-form">
			...
			<li>
				...
				<form:select path="payDateValue" class="input-xlarge"  style="width:190px;">
					<form:option value="" label="全部"/>
	...
	<table id="contentTable" class="table table-striped table-bordered table-condensed">
		...
		<tbody>
		<c:forEach items="${page.list}" var="payrentReport">
			<tr>
				<td>
					${payrentReport.accountName}
				</td>
				<td>
					${payrentReport.projectName}
				</td>
				<td>
					${payrentReport.buildingName}
				</td>
				<td>
					${payrentReport.houseNo}
				</td>
				<td>
					${payrentReport.oriStrucRoomNum}房${payrentReport.oriStrucCusspacNum}厅${payrentReport.oriStrucWashroNum}卫
				</td>
				<td>
					${payrentReport.payDate}
				</td>
				<td>
					${payrentReport.bankName}
				</td>
				<td>
					${payrentReport.bankAccount}
				</td>
				<td>
					${payrentReport.payAmount}
				</td>
			</tr>
		</c:forEach>
		</tbody>
