	...
	<script type="text/javascript">
		$(document).ready(function() {
			$("#btnExport").click(function(){
				$("#searchForm").attr("action","${ctx}/report/sales/exportExpire");
				$("#searchForm").submit();
				$("#searchForm").attr("action","${ctx}/report/sales/expire");
			});
		});
		function page(n,s){
			$("#pageNo").val(n);
...
<body>
	<form:form id="searchForm" modelAttribute="expireReport" action="${ctx}/report/sales/expire" method="post" class="breadcrumb form-search">
	...
	<table id="contentTable" class="table table-striped table-bordered table-condensed">
		...
		<tbody>
		<c:forEach items="${page.list}" var="expireReport">
			<tr>
				<td>
					${expireReport.contractCode}
				</td>
				<td>
					${expireReport.projectName}
				</td>
				<td>
					${expireReport.buildingName}
				</td>
				<td>
					${expireReport.houseNo}
				</td>
				<td>
					${expireReport.roomNo}
				</td>
				<td>
					${expireReport.tenantName}
				</td>
				<td>
					${expireReport.cellPhone}
				</td>
				<td>
					<fmt:formatDate value="${expireReport.startDate}" pattern="yyyy-MM-dd"/>
				</td>
				<td>
					<fmt:formatDate value="${expireReport.expiredDate}" pattern="yyyy-MM-dd"/>
				</td>
				<td>
					${expireReport.remarks}
				</td>
			</tr>
		</c:forEach>
		</tbody>
