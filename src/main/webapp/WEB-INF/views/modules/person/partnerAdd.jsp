				...
				submitHandler: function(form){
					...
					$.get("${ctx}/person/partner/ajaxSave", saveData, function(data){
						...
						if(null!=json.id) {
							...
							iframe.contentWindow.$("[id='partner.id']").find("option").each(function(){
								if($(this).attr("selected")=="selected") {
									$(this).removeAttr("selected");
									return false;
								}
							});
							var text = iframe.contentWindow.$("[id='partner.id']").html();
							text = "<option value='"+json.id+"' selected='selected'>"+json.name+"</option>"+text;
							iframe.contentWindow.$("[id='partner.id']").html(text);
							iframe.contentWindow.$("[id='partner.id']").val(json.id);
							iframe.contentWindow.$("[id='partner.id']").prev("[id='s2id_partner.id']").find(".select2-chosen").html(json.name);
							top.$.jBox.close();
						}
					});
				},
