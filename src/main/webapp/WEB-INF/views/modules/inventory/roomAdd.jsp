			...
			$("#inputForm").validate({
				submitHandler: function(form){
					...
					$.get("${ctx}/inventory/room/ajaxSave", saveData, function(data){
						...
						if(null!=json.id) {
							$(window.parent.document).find("iframe")[0].contentWindow.$("[id='room.id']").find("option").each(function(){
								if($(this).attr("selected")=="selected") {
									$(this).removeAttr("selected");
									return false;
								}
							});
							var text = $(window.parent.document).find("iframe")[0].contentWindow.$("[id='room.id']").html();
							text = "<option value='"+json.id+"' selected='selected'>"+json.name+"</option>"+text;
							$(window.parent.document).find("iframe")[0].contentWindow.$("[id='room.id']").html(text);
							$(window.parent.document).find("iframe")[0].contentWindow.$("[id='room.id']").val(json.id);
							$(window.parent.document).find("iframe")[0].contentWindow.$("[id='room.id']").prev("[id='s2id_room.id']").find(".select2-chosen").html(json.name);
							top.$.jBox.close();
						}
					});
				},
