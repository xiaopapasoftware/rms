...
<body>
	...
	<form:form id="searchForm" modelAttribute="rentContract" action="${ctx}/contract/rentContract/" method="post" class="breadcrumb form-search">
		...
		<ul class="ul-form">
			...
			<li><label>合同来源：</label>
				<form:select path="contractSource" class="input-medium">
					<form:option value="" label=""/>
					<form:options items="${fns:getDictList('')}" itemLabel="label" itemValue="value" htmlEscape="false"/>
				</form:select>
			</li>
			<li><label>月租金：</label>
				<form:input path="rental" htmlEscape="false" class="input-medium"/>
			</li>
			...
			<li><label>合同业务状态：</label>
				<form:select path="contractBusiStatus" class="input-medium">
					<form:option value="" label=""/>
					<form:options items="${fns:getDictList('')}" itemLabel="label" itemValue="value" htmlEscape="false"/>
				</form:select>
			</li>
	...
	<table id="contentTable" class="table table-striped table-bordered table-condensed">
		...
		<c:forEach items="${page.list}" var="rentContract">
			<tr>
				...
				<td>
					${fns:getDictLabel(rentContract.rentMode, '', '')}
				</td>
				<td>
				...
				<td>
					<fmt:formatDate value="${rentContract.startDate}" pattern="yyyy-MM-dd HH:mm:ss"/>
				</td>
				<td>
					<fmt:formatDate value="${rentContract.expiredDate}" pattern="yyyy-MM-dd HH:mm:ss"/>
				</td>
				<td>
					<fmt:formatDate value="${rentContract.signDate}" pattern="yyyy-MM-dd HH:mm:ss"/>
				</td>
				<td>
