		...
		$(document).ready(function() {
			$("#inputForm").validate({
				submitHandler: function(form){
					...
					$.get("${ctx}/inventory/neighborhood/ajaxSave", saveData, function(data){
						var json = eval("("+data+")");
						...
						if(null!=json.id) {
							...
							if(undefined == $(window.parent.document).find(".tab_content").html()) {
								iframe = $(window.parent.document).find(".curholder").find("iframe")[0].contentWindow.$("[id='neighborhood.id']");
							} else {
								iframe = $(window.parent.document).find("[id='jbox-iframe']").contents().find("[id='neighborhood.id']");
							}
...
<body>
	<form:form id="inputForm" modelAttribute="neighborhood" action="${ctx}/inventory/neighborhood/save" method="post" class="form-horizontal">
		...
		<div class="control-group">
			...
			<div class="controls">
				<form:input path="neighborhoodName" htmlEscape="false" maxlength="100" class="input-xlarge required"/>
