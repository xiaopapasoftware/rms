...
@Component
public class CountyLeaseCalculate implements LeaseCalculate {
    ...
    public List<LeaseStatisticsCondition> getChildConditionList(LeaseStatisticsCondition condition) {
        return Optional.ofNullable(areaService.getAreaByParentId(condition.getId(), AreaTypeEnum.CENTER.getValue())).map(list -> list.stream().map(center -> {
            ...
            statisticsCondition.setId(center.getId());
            statisticsCondition.setType(SelectItemConstants.CENTER);
