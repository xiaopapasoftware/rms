...
public class HouseGrossProfitCalculate implements GrossProfitCalculate {
  ...
  public double calculateIncome(GrossProfitCondition condition) {
    ...
    if (!CollectionUtils.isEmpty(contractList)) {
      ...
      double income = paymentTransService.queryIncomePaymentByTransIdAndTime(condition.getStartDate(), condition.getEndDate(), contractIdList).stream().mapToDouble(PaymentTrans::getTradeAmount).sum();
      ...
      for (PaymenttransDtl paymenttransDtl : dtlList) {
        if (TradeDirectionEnum.OUT.getValue().equals(paymenttransDtl.getDirection())) {
  ...
  private double calculateDepositSum(GrossProfitCondition condition) {
    ...
    if (!CollectionUtils.isEmpty(contractList)) {
      ...
      if (depositListCache.getObject(contract.getId()) == null) {
        depositListCache.putObject(contract.getId(), contract.getLeaseContractDtlList().stream().map(dtl -> DataParser.parse(dtl.getStartDate(), dtl.getEndDate(), dtl.getDeposit()))
            .sorted(Comparator.comparing(GrossProfitNumDeposit::getStart)).collect(Collectors.toList()));
      }
      List<GrossProfitNumDeposit> depositList = ((List<GrossProfitNumDeposit>) depositListCache.getObject(contract.getId())).stream()
          .filter(numDeposit -> !(numDeposit.getEnd() < startNum || numDeposit.getStart() > endNum)).collect(Collectors.toList());
