...
public class FrontSearchController extends BaseController {
  ...
  public String search(String t, @RequestParam(required = false) String q, @RequestParam(required = false) String qand, @RequestParam(required = false) String qnot,
      @RequestParam(required = false) String a, @RequestParam(required = false) String cid, @RequestParam(required = false) String bd, @RequestParam(required = false) String ed,
    ...
    if ("cmd:reindex".equals(q)) {
      if (UserUtils.getUser().isAdmin()) {
        ...
        addMessage(model, ViewMessageTypeEnum.SUCCESS, "重建索引成功，共耗时 " + (System.currentTimeMillis() - start) + "毫秒。");
      } else {
        addMessage(model, ViewMessageTypeEnum.ERROR, "你没有执行权限。");
      }
    }
    // 执行检索
    else {
