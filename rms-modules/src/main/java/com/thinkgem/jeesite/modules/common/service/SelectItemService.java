    ...
    private List<SelectItem> getOrgSelectListByCondition(SelectItemCondition condition) {
        ...
        switch (condition.getType()) {
            case SelectItemConstants.company :
                result = convertToItemList(areaService.getCompanyList()
                        .stream()
                        .collect(Collectors.toMap(Area::getId, Area::getName)));
                break;
            case SelectItemConstants.center :
                result = convertToItemList(areaService.getAreaByParentId(condition.getId(), AreaTypeEnum.CENTER.getValue())
                        .stream()
                        .collect(Collectors.toMap(Area::getId, Area::getName)));
                break;
            case SelectItemConstants.area :
                result = convertToItemList(areaService.getAreaByParentId(condition.getId(), AreaTypeEnum.AREA.getValue())
                        .stream()
                        .collect(Collectors.toMap(Area::getId, Area::getName)));
                break;
            case SelectItemConstants.project :
                result = convertToItemList(propertyProjectService.getPropertyProjectByAreaId(condition.getId())
                        .stream()
                        .collect(Collectors.toMap(PropertyProject::getId, PropertyProject::getProjectName)));
                break;
            case SelectItemConstants.house :
                result = convertToItemList(houseService.findHouseListByProjectId(condition.getId())
                        .stream()
